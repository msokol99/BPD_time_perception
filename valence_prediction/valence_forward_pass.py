...
############### SETTINGS ####################
output_base_dir = 'valence_prediction/output_corpora'
input_base_dir = 'input_corpora/controls'

base_model_dir = "sdadas/polish-roberta-base-v2"
trained_model_dir = 'valence_prediction/output_model/valence_model.pth'
...
for subdir, dirs, files in os.walk(input_base_dir):
    for file in files:
        ...
        output_df = pd.DataFrame({'sentence': inputs, 'valence': all_predictions_np})
        ...
        output_file = os.path.join(output_base_dir, os.path.splitext(file)[0] + '_valence.csv')
